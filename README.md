...
**Running any sort of Windows program on Linux is hard**. It's a pain in butt to configure Wine, and for every game or executable you want to run, there's always a bunch of different arguments or flags you have to set that are specific to that program. As a casual gamer who enjoys Linux, it's especially aggravating when buying a new machine. 

_Should I just stick with Windows?_  
Of course you can - and sell your soul to Microsoft in the process. You've also just made any kind of software development or [ricing](https://www.reddit.com/r/unixporn/wiki/themeing/dictionary#wiki_rice) much more tedious and counter-productive.

_I guess I can just dual boot..._  
...
So, of course, I chose to run a virtual machine. But there's still a problem - any virtual machine creates an emulated GPU, meaning that it won't use your machine's GPU. This limits the number of games you can run on your virtual machine to about zero - unless all you wanted to play was MS-DOS games. 

This is something I couldn't compromise on. I use [Unreal Engine 4](https://www.unrealengine.com/en-US/) for game development at my work, which requires a decent GPU. How do you tell a virtual machine to use a hardware GPU?
